<script lang="ts">
  ...
  // ── Search button icon ────────────────────────────────────────────────────
  const BTN_SIZE = BAR_H;   // square button matching bar height
  const GAP = 0.01;
  ...
  (() => {
    ...
    ctx.beginPath();
    ctx.arc(50, 50, 28, 0, Math.PI * 2);   // lens
    ctx.stroke();

    ctx.beginPath();
    ctx.moveTo(72, 72);
    ctx.lineTo(100, 100);                   // handle
    ctx.stroke();
...
<!--
  All children share the same Grabbable group so grabbing the bar
  moves the button with it.
-->
<Grabbable>
  <!-- Text field -->
  ...
  <!-- Search button — Button handles hover/click animation -->
